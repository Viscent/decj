define({
     ...
     "expl.js.details":"decj允许你声明JS模块所依赖的其它JS模块。decj并不再造车轮（Reinvent the wheel）,它默认使用RequireJS实现异步方式动态加载Javascript文件。您也可以使decj使用任何符合AMD（Asynchronous Module Definition）规范的Javascript库来实现Javascript文件动态加载。<br/>声明一个模块所依赖的其它模块，只需要在模块定义中声明各个依赖的ID：",
