define({
     ...
     "expl.i18n":"Declarative Internationalization(I18N) Support:support mutli-language and on-demand dynamic resource file loading",
     ...
     "expl.i18n.details.step2":"Reference to the strings defined in the resource in a page,for example:",
     ...
     "expl.css.details":"decj supports loading CSS files required by a module dynamically.It supports loading locale-specific CSS files.To use this feature,you do not need to write code.Just delcare the CSS files to be loaded in the module declaration: ",
     "expl.css.details.1":"For instance,if the current locale is <i>en_US</i>(English-USA),the above declaration will make <i>form-en_US.css</i> and <i>decj_en_US.css</i> be loaded dynamically.<br/>If the CSS files to be loaded come from a different domain,since many browsers do not provide browser events such as <i>onreadystatechange</i> and <i>onload</i> capable of notifying completion of CSS file loading, decj needs some special \"clue\" to determine whether a CSS is completely loaded or not.Hence, in this case, you have to add a special CSS rule to your external CSS files.The rule should be named as \"#<i>Short name of the CSS file to be loaded</i>-Clue\", and its declaration should contain \"display:none\".For example, your page comes from \"http://www.domain.com\", while a CSS to be loaded comes from \"http://static.domain.com/css/menu.css\", you will have to add a special rule \"#menu-Clue\" to \"menu.css\". ",
     "expl.js":"Declarative Javascript Files on-demand Dynamical Load",
     "expl.js.details":"decj allows you declare a module's dependencies.Those declared dependencies will be loaded dynamically in an async way.Of course,decj does not Reinvent the wheel,it currently uses the RequireJS library to load Javascript file asynchronously.However, you can also make decj use other Javascript library that implements the AMD(Asynchronous Module Definition) specification.",
     ...
     "expl.form.submit":"Form Data Automatic Submission(support JSON)",
     ...
     "expl.form.reset":"Form Reset Enhancement",
     "expl.form.reset.details":"The HTML <i>reset</i> button restores each element in a form to their \"initial value\".Nevertheless,experienced developers may find that if the form is filled programmatically via Javascript(for example,filling with data loaded from server side via AJAX),those \"initial values\" are usually not we expected.<br/>decj enhances the functionality of <bi>reset</i>,making it is possible to fill form with data loaded from server-side and upon clicking of <i>reset</i> button,a form can restore its data to those initial data from which the form is filled with.<br/>To use this feature, you do not need to write code,just add a <i>reset</i> button to your forms.",
     "expl.form.validate":"Form Data Validation",
     "expl.form.validate.details":"If field of a form loses its focus,decj will trigger data validation for the field.<br/>To use this feature,you do not need to write code(unless you want to write your own validation rules),just make configuration.",
     ...
     "expl.form.cfg":"Form Enhancement's Required Common Configurations",
     "expl.form.details.extra":"To implement those form enhancements,decj needs some metadata for a form.These metadata include form fields' data type,validation rules and formatting requirements,and some other extra metadata.<br/>This demo provides below metadata for form fields(actually these metadata can be generated by server-side code):",
     "expl.form.details.extra.1":"More-ever,you can provide some extra metadata for a form.",
     "expl.form.code.comment.1":"Declare some extra processing for form data submission.",
     ...
     "expl.load":"Concurrently Loading of HTML Code/CSS Files/Javascript Files/Resource Files",
     "expl.load.details":"decj loads stuff required like HTML code,CSS files,Javascript files and resource files on-demand and concurrently,to speed up the loading.",
     "expl.init":"Declarative Page/Module Initialization",
     "expl.init.details":"In most cases,after a page/module being loaded,some initializing code should be executed, these code can be declared in a module definition,like:",
