  ...
  */
  ...
  HTMLFormRef.of = function(htmlForm) {
    ...
    if (undefined == instance) {
      ...
      HTMLFormRef._instances[id] = instance = new HTMLFormRef(htmlForm);
      instance._id = id;
    }
    return instance;
  };
...
//end of utility function declaration
  ...
  var _decjConfig = {
    /*extensible*/validationNotify: function(objField, msg) {
  ...
  FieldMetaData.prototype = {
    attr : function(fieldName, attr) {
      ...
      if (undefined == fldCfg) {
        throwErr("FieldMetaData.attr", "No such field " + fieldName + "!");
      }
      var v = fldCfg[attr];
      switch (attr) {
      case 'stereoType':
  ...
  StereoType.types = {
    ...
    "Number" : {
      ...
      formatter : function(value, param) {
        ...
        if (rounding) {
          ...
          for ( var i = 0; i < decimalCount; i++) {
            tmpDecimalPart.push(decimalPart.charAt(i) || "0");
          }
      ...
      isValid : function(value, param) {
        var result = true,
        p = $.extend(this.getDefaultTypeParam(),(param || {})),
        locale = p.locale || i18n.getLocale(),
        decimalSep = _numericFormat.getCfg(locale).decimalSep,
        thousandSep = _numericFormat.getCfg(locale).thousandSep;
        ...
        partsCount=tmpArr.length;
        if(partsCount>=3){
          result = false;
        }else{
          var integralCountLimit = p.integer,
          decimalCountLimit = p.fraction,
          integralPart = tmpArr[0],
          decimalPart = tmpArr[1] || '0';
    ...
    "Date" : {
      parser : function(value, param) {
        ...
        var date = new Date((2 == position.y[1]) ? 
            parseInt(currYear - (currYear % 1000)) + parseInt(year) : year, month - 1, day);

        var year1 = (2 == position.y[1]) ? date.getFullYear() % 1000 : date.getFullYear();
        var valid = (year == year1 && (month == date.getMonth() + 1) && day == date.getDate());
        if (!valid) {
      ...
      formatter : function(value, param) {
        ...
        year = (-1 != fmt.indexOf('yyyy')) ? date.getFullYear() : (date.getFullYear() % 1000);
  ...
  StereoType.of = function(typeName) {
    ...
    var result = cache[typeName] || (cache[typeName] = new StereoType(typeName));
  ...
  function bindEventHandlers(timerId,module) {
    //Older version of Firefox may passin a timer Id as the 1st argument,if the function is called from setTimeout
    module=module || timerId;
    ...
    for ( var bindingKey in eventBindings) {
      ...
      if ($.isPlainObject(handler)) {
        handler = handler.h.transform(handler.extra, handler.ctx || null);
      }
  ...
  Form.validateField = function(field, rules, typeParam,validatorRegistry) {
    ...
    for ( var ruleName in rules) {
      ...
      if (!result.valid) {
        if (_debug) {
          debug("Validation rule " + ruleName + " NOT passed for field " + field.name);
        }
        break;
      }
    }//end of for
  ...
  REG_EXP_LOCALE_DELIMITER = /[^a-z]/i;
