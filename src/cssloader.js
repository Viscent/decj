...
CSSLoader.prototype={
  ...
  _loadCSS:function(urls){
    ...
    if(isCrossDomain(url)){
      ...
      $.get(url).done(this._cssAjaxLoaded.transform([url,this.cfg.ownerModule],this))
          .fail(this._cssAjaxFailed.transform(url,this));
    }
  },
...
function isCrossDomain(url){
  ...
  matches=(url || '').match(REG_EXP_HOST),
  result=false;
...
function addParam(url,param,anchor){
  ...
  var parts=url.split("#"),
  url=parts[0],
  anchorPart=(1==parts.length) ? "" : "#"+parts[1];
  parts=url.split("?");
